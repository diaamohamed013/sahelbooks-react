...
// add password strength word trans

const weak = document.querySelector('.msg_weak').value;
const moderate = document.querySelector(".msg_moderate").value;
const strong = document.querySelector(".msg_strong").value;
const extraStrong = document.querySelector(".msg_extraStrong").value;
...
const unblockSubmitBtn = (btn, strength) => {
  ...
  if (strength === 'none' || strength === 'weak') {
    btn.disabled = true;
  } else if(!$(".agree_btn").length || $(".agree_btn").is(":checked")) {
    btn.disabled = false;
  }
